'''
 use forget_remember mechanism and CDD2 method to find the seed
 coded by YOU
'''
...
def cdd2_dynamic_linear(G_snapshots, k, num, min_distance):
    '''
    cdd method with linear forgetting : sum( l * grade(l)^l ),
    use (1+cluster) * (degree of nodes) to grade nodes
    ...
    for i in range(num):
        ...
        for u in nodes_use:
            grade = -(1 + nx.clustering(graph, u)) * degree[u]
            grade_dict[u] = grade
...
def cdd2_dynamic_hyp(G_snapshots, k, num, min_distance):
    '''
...
def cdd2_dynamic_exp(G_snapshots, k, num, min_distance):
    '''
